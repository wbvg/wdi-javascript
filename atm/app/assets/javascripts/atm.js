    ...
    var cheque_withdrawl = parseInt($('#cheque_withdrawl').val());
        $('#chq_balance').text(chq_balance);
         if (chq_balance > cheque_withdrawl) {
          ...
          alert('Insufficient funds').addClass('alert success');
         }

/* Subtraction - Borrow from Savings*/
        else if (chq_balance < 0 && chq_balance < cheque_withdrawl) {
        $('#chq_balance').addClass('borrow');
         overdraft = parseInt($('#balance').text(balance);
         chq_balance = (balance - overdraft);
        } else {
          $('#chq_balance').addClass('alert');
          alert('Insufficient funds in Savings').addClass('alert');
         }

/* End - Borrow from Savings*/
    }
